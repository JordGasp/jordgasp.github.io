...
- **Daemon readiness polling** with timeout (90s max)
...
- **Digest-based update detection**: compares the local image against what Docker Hub actually publishes under `:latest` (via the Docker Hub API) before downloading anything - avoids false positives from GitHub releases that haven't been pushed to Docker Hub yet
...
│   └── n8n-control.ps1          # management script (860 lines)
...
On each interactive start, the script checks whether Docker Hub's `n8nio/n8n:latest` tag actually points to a different image than what's cached locally - **before downloading anything**:

1. **Query the Docker Hub registry API** for the `:latest` tag's manifest digest (a small metadata call, no image layers transferred)
2. **Compare against the local image's digest** (`docker inspect ... RepoDigests`)
3. If they match, start immediately - nothing to do
4. If they differ, **fetch GitHub releases** via REST API for changelog context (informational only - GitHub tags can lag behind what's actually published to Docker Hub, so it's never used to *decide* whether an update exists)
5. **Detect breaking changes** using regex pattern matching:
    - Keywords: `breaking`, `removed`, `deprecated`, `migration`, `incompatible`
6. **Display warnings** if breaking changes are found, plus a changelog overview
7. **Prompt user** for confirmation (skipped entirely in `-AutoStart` mode, which never applies updates unattended)
8. **Create a safety backup** - reused automatically if the scheduled auto-backup already ran earlier in the same start, to avoid backing up twice
9. **Pull the image and check the exit code** - if the download fails, the update is aborted instead of silently recreating the container with the unchanged old image
10. **Force recreate container**, then **re-read the actual running version** to confirm and report what's really installed

This design replaced an earlier version that pulled first and diffed local image IDs, which could falsely report an update as applied when the download had failed, or loop on a version GitHub had tagged but Docker Hub hadn't published yet.
...
4. **Safety backup of the current data** taken automatically first, so a bad restore or an accidental wrong pick can't destroy the current state
5. **Container stop** before extraction
6. **Temporary extraction** to `_restore_temp` directory
7. **Validation** of extracted structure (the data folder is detected by name from the configured path, not hardcoded)
8. **Atomic swap** of data directory
9. **Cleanup** of temporary files
10. **Container restart** with status verification
...
| `PS1_CONTAINER_NAME`               | `n8n-docker`            | Container name (shared with compose)    |
